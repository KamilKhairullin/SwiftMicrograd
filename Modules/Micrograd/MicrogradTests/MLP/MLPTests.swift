...
final class MLPTests: XCTestCase {
    ...
    func testLayer() throws {
        ...
        let mlp = MLP(layers: [layer])
        let result = mlp.call(input: x)
    ...
    func testNeuron_parameters_change() throws {
        ...
        XCTAssert(mlp.layers[0].neurons[0].weights.map { $0.data } == [0.001, 0.3, 0.5, 0.7])
        XCTAssert(mlp.layers[0].neurons[0].weights.map { $0.gradient } == [0.0, 0.5505, 0.0, 0.0])
    }
}
